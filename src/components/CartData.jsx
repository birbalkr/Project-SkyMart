...
function CartData({ setIsCartOpen, isCartOpen }) {
    ...
    let { cartSaveData } = useContext(MyStore);
    return (
        <div
        ...
        >
            ...
            {cartSaveData.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-center px-6 h-[70%]">
                    ...
                    <button className="bg-lime-400 hover:bg-lime-300 text-black font-semibold px-6 py-3 rounded-xl transition-colors" onClick={() => setIsCartOpen(false)}>
                        Browse Products
                    </button>
                </div>) : (
                <div className='flex-1 overflow-y-scroll h-screen'>
                    {cartSaveData.map((item) => {
                        return <AddCartPage key={item.id} cart={item} />
                    })}
                </div>
            )}
        </div>
